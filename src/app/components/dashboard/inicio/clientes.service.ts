...
export class ClientService {
  ...
  async addClient ( client : createClient ) {
    const { nombre, apellidos, ncuenta, ultimoPago, fechaVencimiento, numeroCelular, numeroCelularEmergencia, precioConvenio, gender, pass } = client;
    let tipoMensualidad = client.tipoMensualidad;
    if (tipoMensualidad === "Convenio") tipoMensualidad = `Convenio-./()/.-${precioConvenio}`
    return this.http.post(this.URL, { nombre, apellidos, ncuenta, ultimoPago, fechaVencimiento, tipoMensualidad, numeroCelular, numeroCelularEmergencia, gender, pass });
  }
  ...
  async updateClient ( _id : string, client : updeateClient, profileImg : string ){
    const { nombre, apellidos, ncuenta, tipoMensualidad, numeroCelular, numeroCelularEmergencia, gender, pass } = client;
    let updateObject : any = { nombre, apellidos, ncuenta, tipoMensualidad, numeroCelular, numeroCelularEmergencia, gender, pass };
    if (profileImg) updateObject = { ...updateObject , profileImg }
    return await this.http.put(this.URL+`/${_id}`, updateObject );
  }

  async renovarMensualidad ( _id : string, client : renewMensualidad ){
    const { tipoMensualidad, ultimoPago, fechaVencimiento } = client;
    console.log(client);

    return await this.http.put(this.URL+`/${_id}`, { tipoMensualidad, ultimoPago, fechaVencimiento });
  }

  async deleteClients ( _id : string ) {
    return await this.http.delete(this.URL+`/${_id}`);
  }

  async findOneClientByCuenta(ncuenta:string){
    return await this.http.get(this.URL + `/cuenta/${ncuenta}`)
  }
